...
**Status:** identity, multi-account support, X3DH + Double Ratchet end-to-end encrypted 1:1 messaging, invite QR codes, push notifications (via [UnifiedPush](https://unifiedpush.org/)), and a server admin area are implemented. Android only for now; groups/broadcast and federation are future work, tracked alongside the [server](https://github.com/behringer24/freizone-server)'s own roadmap.
...
- **Push notifications** while the app is closed, via UnifiedPush — no Google-only dependency, works with any compatible distributor (e.g. [ntfy](https://ntfy.sh/)). The push payload itself carries no content or metadata; it's purely a "go sync" wake-up.
...
lib/
  ...
  push/     UnifiedPush wiring + local "new message" notifications
